...
VOID printTaintHeap(){
	std::cout << std::hex << "\n\t" << "*****************Tainted memory in heap section*****************" << std::endl;
	int count=0;
	list<UINT32>::iterator i;
	addressTainted.unique();
	for(i = addressTainted.begin(); i != addressTainted.end(); i++){
...
VOID printCT(){
	std::cout << std::hex << "\n\t" << "*****************Tainted CT Address*****************" << std::endl;
	int count=0;
	list<UINT32>::iterator i;
  addressCT.unique();
	for(i = addressCT.begin(); i != addressCT.end(); i++){
		if(*i>=0x8048000&&*i<=0x804b000){
			std::cout << std::hex<<"\t"<< *i;
...
bool taintReg(REG reg)
{
  ...
  switch(reg){
    ...
    //case REG_SIL:  regsTainted.push_front(REG_SIL); 
         break;
    case REG_EFLAGS: regsTainted.push_front(REG_EFLAGS);
         break;

    case REG_XMM0: regsTainted.push_front(REG_XMM0);
    	 break;
    case REG_XMM1: regsTainted.push_front(REG_XMM1);
    	 break;
    case REG_XMM2: regsTainted.push_front(REG_XMM2);
    	 break;
    case REG_XMM3: regsTainted.push_front(REG_XMM3);
    	 break;
    case REG_XMM4: regsTainted.push_front(REG_XMM4);
    	 break;
    case REG_XMM5: regsTainted.push_front(REG_XMM5);
         break;
...
bool removeRegTainted(REG reg)
{
  switch(reg){
    ...
    case REG_EFLAGS: regsTainted.remove(REG_EFLAGS);
         break;

    case REG_XMM0: regsTainted.remove(REG_XMM0);
    	 break;
    case REG_XMM1: regsTainted.remove(REG_XMM1);
    	 break;
    case REG_XMM2: regsTainted.remove(REG_XMM2);
    	 break;
    case REG_XMM3: regsTainted.remove(REG_XMM3);
    	 break;
    case REG_XMM4: regsTainted.remove(REG_XMM4);
    	 break;
    case REG_XMM5: regsTainted.remove(REG_XMM5);
         break;
...
VOID ReadMem(UINT32 insAddr, std::string insDis, UINT32 OperandCount,REG reg_r, UINT32 memOp, UINT32 sp)
{
  ...
  if (checkAlreadyRegTainted(reg_r)){
  	if(insAddr<=0x8049000)
    	std::cout << std::hex <<insAddr<< ":\t"  <<"[READ in " << addr << "][F]" <<" insDis:"<<insDis<< std::endl;
...
VOID WriteMem(UINT32 insAddr, std::string insDis, UINT32 OperandCount,REG reg_r, REG reg_0,UINT32 memOp, UINT32 sp)
{
  ...
  if (checkAlreadyRegTainted(reg_r)){
  	if(insAddr<=0x8049000)
		std::cout << std::hex << insAddr<< ":\t"  <<"[WRITE in " << addr << "][T]" << " insDis:"<<insDis<< std::endl;
    UINT32 length=0;
    if(REG_is_Lower8(reg_r)||REG_is_Upper8(reg_r)){
    	length=1;
    }else if(REG_is_Half16(reg_r)){
...
VOID CT(UINT32 insAddr, std::string insDis, UINT32 OperandCount, REG reg_r, REG reg_w)
{
  if (REG_valid(reg_w)||REG_valid(reg_r)){
  	if (checkAlreadyRegTainted(reg_r)||(checkAlreadyRegTainted(reg_w)))
    {
    	addressCT.push_back(insAddr);
    }
  }  
}
...
VOID Instruction(INS ins, VOID *v)
{
  ...
  else if (INS_Opcode(ins)== XED_ICLASS_CMP || INS_Opcode(ins)== XED_ICLASS_TEST){
     		INS_InsertCall(
        ins, IPOINT_BEFORE, (AFUNPTR)CT,
        IARG_ADDRINT, INS_Address(ins),
        IARG_PTR, new string(INS_Disassemble(ins)),
        IARG_UINT32, INS_OperandCount(ins),
  ...
  /*if ( INS_IsCall(ins)&&INS_OperandReg(ins, 0)==REG_EAX){
...
VOID Syscall_entry(THREADID thread_id, CONTEXT *ctx, SYSCALL_STANDARD std, void *v)
{
  ...
  /*
  if (PIN_GetSyscallNumber(ctx, std) == 45){
      start = static_cast<UINT64>((PIN_GetSyscallArgument(ctx, std, 1)));
      size  = static_cast<UINT64>((PIN_GetSyscallArgument(ctx, std, 2)));
      for (i = 0; i < size; i++)
        addressTainted.push_back(start+i);
